...
export default function Home(){
 ...
 return <main className="shell">
  ...
  <section className="grid" id="generate">
   ...
   <div className="card" id="keys"><h2>🔑 Customer API key</h2><p className="muted">ViralMovie creates the key. The provider key stays server-side. If this email matches the private OWNER_EMAIL setting, the key uses Owner Mode.</p><label>Name</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="My app"/><label>Email</label><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" type="email"/><button className="btn" onClick={createKey}>Create API key · FREE</button>{newKey&&<div className="status"><b>Save your API key now</b><div className="code" style={{marginTop:8,wordBreak:"break-all"}}>{newKey.key}</div><p className="muted">{newKey.owner ? "Owner mode: unlimited platform credits for your owner email." : "10 free credits included."}</p></div>}</div>
  </section>
