...
import {createKey,isOwnerEmail} from "@/lib/store"
...
export async function POST(req:Request){
 try{
  ...
  return NextResponse.json({ok:true,...result,owner:isOwnerEmail(email),message:isOwnerEmail(email)?"Owner API key created. Video generation will not consume platform credits for this email.":"Save this API key now. It will not be shown again."})
